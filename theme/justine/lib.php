 ...
 * the {@link theme_justine_less_variables()} here because we need to create selectors or
 * alter existing ones.
 *
 * @param theme_config $theme The theme config object.
 * @return string Raw LESS code.
 */
function theme_justine_extra_less($theme) {
    $content = '';
 ...
 */
function theme_justine_less_variables($theme) {
    $variables = array();
    ...
    if (!empty($theme->settings->h2color)) {
        $variables['h2color'] = $theme->settings->h2color;
    if (!empty($theme->settings->hovercolor)) {
        $variables['hovercolor'] = $theme->settings->hovercolor;
    }
 ...
 */
function theme_justine_process_css($css, $theme) {
    global $OUTPUT;

    // Set the background image for the logo.
    $logo = $OUTPUT->get_logo_url(null, 100);
    $css = theme_justine_set_logo($css, $logo);
    ...
    $css = theme_justine_set_customcss($css, $customcss);
 ...
 */
function theme_justine_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_justine_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM &&
            ($filearea === 'logo' || $filearea === 'smalllogo' || $filearea === 'backgroundimage')) {
        $theme = theme_config::load('justine');
 ...
 */
function theme_justine_set_customcss($css, $customcss) {
